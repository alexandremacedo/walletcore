...
func (a *AccountDB) FindByID(id string) (*entity.Account, error) {
	...
	stmt, err := a.DB.Prepare("SELECT a.id, a.client_id, a.balance, a.created_at,c.id,c.name,c.email,c.created_at FROM account a INNER JOIN clientes c ON c.id = a.client_id WHERE a.id = ?")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
